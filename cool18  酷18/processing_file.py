...
async def remove_html(text):
    ...
    text = re.sub(r'</p>', '\n', text)
    text = re.sub(r'<b>(.*?)</b>', '\n', text)
    return str(BeautifulSoup(text,'lxml').text)
...
# 保存文章
async def processing_data(html, new_dir_name,byte,index,Currect_tid):
    try:
            ...
            if await check_sensitive(article_name):
                ...
                if follow_up:  # 存在回帖
                    ...
                    if len(first_pre_article_content) < 2000:  # 字数少，跳过
                        print(first_pre_article_content)
                        valid = False
...
# 消费者，处理网页资源
async def consumer(file_name,new_dir_name,semaphore,byte,index):
        ...
        if not html.select_one('head>script') and not html.select_one('td.show_content>p'):
            ...
            try:
                await asyncio.wait_for(
                    processing_data(html, new_dir_name,byte, index, Currect_tid), 7200)
            except asyncio.TimeoutError:
                # 撰写错误报告
                message = f'【{str(datetime.datetime.now())[:16]}】：consumer()函数\n' \
                          f'【{str(datetime.datetime.now())[:16]}】：超过2小时，文章处理未完成处理\n' \
...
async def consumers(concurrency_num: int,new_dir_name):
    ...
    tasks=[]
    num=0
    # tasks.append(consumer('14313265.html',new_dir_name,semaphore,byte,del_bracket,index))
    for x in sorted(files):
        print(f'【{str(datetime.datetime.now())[:16]}】：创建访问列表，列表已添加 {num} 文件',end='\r',flush=True)
        num+=1
...
async def main():
    ...
    if os.path.exists(target_path):
        ...
        print('禁忌书屋目录并不存在！请确保脚本路径存在包含所有html文件的\"禁忌书屋\"文件夹')
        sys.exit()
